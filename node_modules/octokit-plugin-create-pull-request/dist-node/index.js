...
async function createTree(state, changes) {
  ...
  }))).filter(Boolean); // https://developer.github.com/v3/git/trees/#create-a-tree
...
async function createCommit(state, changes) {
  const {
    octokit,
    repo,
    fork,
    latestCommitSha
  } = state; // https://developer.github.com/v3/git/commits/#create-a-commit
...
async function octokitCreatePullRequest(octokit, {
  owner,
  repo,
  title,
  body,
